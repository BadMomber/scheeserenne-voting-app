<template>
  <div>
    <!-- <b-container v-if="!voterByIp && acceptedTerms === false" id="pre-content">
      <b-row>
        <b-col cols="12 mt-5">
          <h3>
            Bitte akzeptieren sie die Bedingungen, um an dem öffentlichen Voting
            teilzunehmen
          </h3>
        </b-col>
        <b-col cols="12 mt-5">
          <b-button variant="success" @click="setTermsAccepted">
            <span class="text-uppercase font-weight-bold">Annehmen</span>
          </b-button>
        </b-col>
      </b-row>
    </b-container> -->
    <b-container
      v-if="acceptedTerms === true || voterByIp !== undefined"
      id="main-content"
    >
...
export default {
  ...
  methods: {
    ...
    async setTermsAccepted(e) {
      this.acceptedTerms = await true
      const accepted = await this.acceptedTerms
      console.log("terms accepted:", this.termsAccepted)
      this.addVoter(accepted, e)
    },
    async addVoter(acceptedTerms, e) {
      e.preventDefault()
      ...
      try {
        await this.$apollo.mutate({
          variables: {
            termsAccepted: this.acceptedTerms,
          },
          mutation: gql`
    ...
    calculatePoints(evt) {
      ...
      // console.log("name:", evt.moved.element.name, evt.moved.element.id)
      // console.log("calculate points:", this.pointMapper(evt.moved.newIndex))
      // console.log("scheeseList:", this.scheeseList)
    },
    async addVotes(e) {
      e.preventDefault()
      this.error = null
      try {
        await this.scheeseList.forEach(async (scheese, index) => {
          console.log("_points:", index)
          await this.$apollo.mutate({
            variables: {
              scheeseId: scheese.id,
              points: this.scheeseList.length - index,
            },
            mutation: gql`
