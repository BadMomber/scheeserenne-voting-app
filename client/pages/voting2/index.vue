        ...
        <b-col cols="12">
          <b-form>
            <label class="mt-1" for="new-scheese-name"
              >Abstimmungscode hier eingeben {{ voter_hash }}
            </label>
            <b-form-input
              placeholder="Code..."
              id="new-scheese-name"
  ...
  methods: {
    ...
    sortRanked() {
      ...
      for (const scheese of this.votingTwoByVoterCode) {
        ...
        const found = this.scheeseListTwo.find(
          element => element.id === scheese.scheeseId
        );
        console.log("found:", found);
        this.ratedScheese.push(found);
        this.scheeseListTwo.splice(this.scheeseListTwo.indexOf(found), 1);
        // this.scheeseListTwo.splice(this.scheeseListTwo.map((e) => (e.id)).indexOf(scheese.id))
      }
    },
    setActive() {
    ...
    sort() {
      this.scheeseListTwo = this.scheeseListTwo.sort(
        (a, b) => a.order - b.order
      );
    },
    pointMapper(index) {
    ...
    calculatePoints() {
      ...
      if (this.ratedScheese.length > 3) {
        ...
        this.scheeseListTwo.push(lastElement);
      }
    },
